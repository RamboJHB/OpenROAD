...
**场景**:上下两行各自都有一条**够宽**的同 VT(L)区,但**错开**,只重叠很少几列:

```
        列2  列3  列4  列5  列6  列7  列8  列9
行0:    L    L    L    L    L    L    .    .      行0 的 L = 列2-7(宽 6)
行1:    .    .    f:X  f:X  L    L    L    L      行1 的 L = 列6-9(宽 4)
                       ↑↑   ←── 上下只在列6-7 交叠(桥宽 2)
```

**为什么是违例**:行0 的 L 和行1 的 L 各自水平都 ≥ ωMW,**单看每行都没事**;但把
它们当成**同一块 implant**(union)来看,连接上半和下半的「桥」只有列6-7 两列宽。
若 ωMW = 3,这个 **2 宽的跨行颈** < ωMW → MW 违例。
- 这正是我**原模型漏掉**的情形:列6-7 在行0 里属于一条 6 宽的水平 run,被
  「水平 run ≥ ωMW」误判通过。正确判法是看**相邻行的交叠列段宽度**(见 §6 (b))。

**算法怎么修**:把行1 里挡在中间的 `f:X` 改成 L,**加宽交叠**:

```
        列2  列3  列4  列5  列6  列7  列8  列9
行1:    .    .    L    L    L    L    L    L      X→L 后行1 的 L = 列4-9
                  ←──── 现在上下交叠 = 列4-7(宽 4)≥ ωMW,颈消失 ────→
```

- 把 `f:X`(列4-5)换成 L 后,行1 的 L 左移到列4,和行0(列2-7)的交叠变成列4-7
  共 4 列 ≥ ωMW → 跨行颈消失。前提是列4-5 确实是**可改的 filler**且库里有 L;
  若那里是**固定 cell** 或库缺 L → 改不动 → 残留回报上游(同行为 4)。
