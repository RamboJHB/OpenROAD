...
3. **选权重最高、且目标 VT 可实现**的候选 → `replaceFillerVt`(同尺寸换 master,
   ...
   - **平票规则(已定稿)**,依次比较:
     ...
     3. 还相同 → **同 VT 邻居数 y 更小**者(y = 该 filler 四边相邻同 VT 的格数;
        y 越小越孤立、越该改);
     4. 还相同 → **grid 坐标靠左者**(col 最小;再相同则 row 最小)。
     不同 filler 的 (col,row) 必不相同,所以**总能确定唯一胜者**(不再因平票
     unfixable)。实现:`VtRepair::chooseCandidate`。
...
- **平票已定稿**(§3 步骤 3):权重 → 更窄 → 同 VT 邻居数 y 更小 → grid 坐标靠左
  (col,row);总能确定唯一胜者。
