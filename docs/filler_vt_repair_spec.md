...
3. **选权重最高、且目标 VT 可实现**的候选 → `replaceFillerVt`(同尺寸换 master,
   orientation 随行)。
   - **平票规则**:权重相同 → 改**更窄**的那个 filler(更像孤岛、改动更小)。
   - **⚠️ 未决(open)**:权重相同**且宽度也相同** → 改哪个还没定。代码当前用
     **确定性的最小 id 兜底**(`take = id < best.filler`),待你定夺(见 §7)。
...
- **⚠️ 待决:平票且同宽**(§3 步骤 3):两个候选权重相同、宽度也相同时,该改哪个还没
  想好。当前代码用确定性的**最小 id 兜底**(只为可复现),**不是最终规则**。可能的方向:
  看哪个改完邻接更连续 / 看左右 cell 的 orientation/VT 哪个更顺 / 随机但可复现。待定。
