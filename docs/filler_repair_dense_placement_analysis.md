...
checker/engine regression(99)与 portable checker density(71)的完整 normal 与
ASan CTest 为 **252/252**，并在 `-Wall -Wextra -Werror` 下编译通过。
...
100% occupancy 设计上，而不是 60-site fixture 上；本轮用 252/252 保持全绿来证明
...
语义、不动 engine 调用方式与任何接口数据结构，靠 252/252 normal/ASan CTest
