...
`CheckResult.status == CheckStatus::Checked && CheckResult.isLegal == true &&
CheckResult.violations.empty()`。
residual/new/spillover 分类只做 best-effort diagnostics、排序提示和未来增强,不作为第一版正确性依赖。
...
6. 每个 overlay 方案都调用 DP checking API,让 checker 在该局部 window/cluster 上重新检查,
   不 commit DB。
7. filler engine 先用 checker-clean 判定 accept/reject:`status == CheckStatus::Checked`,
   `isLegal == true`,且本次 `violations` 为空才接受。original/residual/new/spillover
   分类在第一版只用于 debug、排序提示和扩窗解释。
...
- checker RD:只做 non-mutating overlay DRC verify。输入 target std-cell + 多个 filler
  change candidates,输出每个 candidate 的真实 violation snapshot。
...
- filler repair engine:只做 window/cluster、候选排序、overlay 搜索和 checker-clean 收敛。
...
struct Violation
{
    ...
    // Inter-row violation must report both rows. Intra-row reports one row.
...
signature,但最终 correctness 只看 checker-clean。

### 3.2 Checker RD API

checker 的主接口是 batch overlay check。candidate 里不包含 `TargetPlace`,避免一个 batch
里混多个 anchor。结果顺序必须和 candidates 输入顺序一一对应。

```cpp
struct OverlayCandidate
{
    int candidateId = -1;  // repair engine debug id
...
struct CheckResult
{
    int candidateId = -1;  // echo OverlayCandidate.candidateId
...
- 一个 `OverlayCandidate` 可以包含多个 `FillerChange`,支持 MW/bridge group move。
- 返回 vector 顺序与 `request.candidates` 一一对应;`candidateId` 只是 debug echo。
- 单个非法 candidate 只影响对应 `CheckResult`,不影响同 batch 其他 candidate。
- `status != Checked` 时必须给 diagnostics;repair 会拒绝该 candidate。
...
如果 checker RD 希望保留 single-overlay convenience wrapper,可以额外提供:
...
可选未来扩展:如果需要显式边界/spillover collect,在 `OverlayCheckRequest` 上加
`std::optional<Rect> guardRegion`。`guardRegion` 只控制 checker 额外收集 violations 的区域,
不表示 repair 可以修改 guard-only 区域里的 filler。
...
这样 skeleton 可以先用 fake checker / fake infrastructure 单测 `isCheckerClean`、candidate
sorting、group seed、beam 和 diagnostics;真实 adapter 等各 RD 接口稳定后再接入。
...
`targetPlace` 做一次单元素 batch check。clean 判定仍然是
`status == CheckStatus::Checked && isLegal == true && violations.empty()`。若未来 checker 支持 `guardRegion`,最终 check
...
1. **第一版只接受 strict checker-clean**。
   `status == CheckStatus::Checked && isLegal == true && violations.empty()` 是唯一
...
每个 overlay 都通过 batch checker API 得到真实 checker result。第一版
...
2. 非 clean 中,`checkerError=false` 优先;checker 未完成的 candidate 只保留 diagnostics,不参与成功候选。
...
- `hasSolution=true`: `changes` 经最终 full overlay 检查满足
  `result.status == CheckStatus::Checked && result.isLegal && result.violations.empty()`。
...
9. 实现 `isCheckerClean(result)` gate:
   `status == CheckStatus::Checked && isLegal == true && violations.empty()`。
...
14. 加 fake checker 单测:
   ...
   - batch API 返回顺序与 `OverlayCandidate` 输入顺序一一对应;
...
需要 checker RD 提供一个 non-mutating batch overlay check。推荐接口:
