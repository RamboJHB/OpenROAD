...
// -----------------------------------------------------------------------------
// test_filler_repair -- drive the filler VT repair chain on the loaded design.
//
// For every movable placed cell it re-checks the CURRENT master through
// DePlace::isLegal (checkDRC -> ImplantLayerChecker::check). On a DRC-illegal
// cell the checker consults the lazily-initialized FillerRepairEngine; when a
// checker-verified filler swap set exists, isLegal comes back true and the
// records are appended to the caller-owned fcRecord. Nothing is committed:
// the command only reports what the engine proposes.
// -----------------------------------------------------------------------------
bool TestFillerRepairCmd::exec()
{
  std::cout << "========================================\n";
  std::cout << "  test_filler_repair: checker + repair  \n";
  std::cout << "========================================\n";
  ...
  // set_filler_option must have run first: the repair engine's candidate
  // universe is exactly this allow list.
  fillerSetting* setting = de_place->getFillerSetting();
  if (!setting || setting->getFillerPhysCells().empty()) {
    std::cout << "ERROR: no filler masters configured "
                 "(run set_filler_option first)\n";
    return false;
  }
  std::cout << "configured filler masters: "
            << setting->getFillerPhysCells().size() << "\n";

  const auto libCellName = [design](LibCellID lcId) {
    const PhysLibCell& cell = design->getLibAcc().getPhysLibCell(lcId);
    return cell.getLibCell().getName();
  };
  ...
  int repairable = 0;
  int proposedSwaps = 0;
  for (auto& node : de_place->getNetwork()->getNodes()) {
    ...
    std::vector<FillerCellRecord> fcRecord;
    const bool legal = de_place->isLegal(cellId, currentLcId, fcRecord);
    if (legal && fcRecord.empty()) {
      continue;  // clean as-is, nothing proposed
    }
    if (!legal) {
      ++illegal;
      std::cout << "  DRC-illegal (no filler repair found): node="
                << node->getId() << " master="
                << libCellName(currentLcId) << " pos=("
                << node->getLeft().v << "," << node->getBottom().v << ")\n";
      continue;
    }
    // legal AND records present: the repair engine proposed a swap set.
    ++repairable;
    proposedSwaps += static_cast<int>(fcRecord.size());
    std::cout << "  repairable: node=" << node->getId() << " master="
              << libCellName(currentLcId) << " swaps=" << fcRecord.size()
              << "\n";
    for (const FillerCellRecord& record : fcRecord) {
      std::cout << "    swap filler cell=" << record.cell_id_.getIndexValue()
                << " at (" << record.origin_x_.getStorage() << ","
                << record.origin_y_.getStorage() << ") "
                << libCellName(record.orig_lib_cell_) << " -> "
                << libCellName(record.new_lib_cell_) << "\n";
    }
  }

  std::cout << "\n--- Result ---\n";
  std::cout << "  checked: " << checked << "  illegal: " << illegal
            << "  repairable: " << repairable
            << "  proposed swaps: " << proposedSwaps << "\n";
  const bool passed = illegal == 0;
  std::cout << "\n========================================\n";
