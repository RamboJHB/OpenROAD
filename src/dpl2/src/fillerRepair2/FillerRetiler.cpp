 ...
 private:
  void search(std::size_t coveredCount)
  {
    ...
    for (const FillerFootprint& footprint : footprints_) {
      ...
      for (const std::size_t cell : cells) covered_[cell] = true;
      current_.push_back(
          TiledFiller{footprint.masterId, anchor.rowId, anchor.colId});
      search(coveredCount + cells.size());
      current_.pop_back();
      for (const std::size_t cell : cells) covered_[cell] = false;
      if (result_.solutions.size() >= config_.maxSolutions
                                ...
                                RetileConfig config)
{
  ...
  emptySites.erase(std::unique(emptySites.begin(), emptySites.end()),
                   emptySites.end());
  footprints.erase(
      std::remove_if(footprints.begin(), footprints.end(),
  ...
  footprints.erase(
      std::unique(footprints.begin(), footprints.end(),
