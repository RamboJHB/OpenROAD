    ...
    const char* requestSource)
{
  ...
  const OpType targetOp = request.targetOp;
  ...
  if ((hasOldTarget
       && static_cast<InstanceId>(request.instanceId) != inst->id)
  ...
  const std::optional<eUNL::LeafCellID> targetCell
      = hasOldTarget
            ? std::optional<eUNL::LeafCellID>(
  ...
  const RowId targetRowId
      = hasOldTarget ? inst->rowId : static_cast<RowId>(request.rowId);
  const DbCoord targetX
      = hasOldTarget
            ? inst->x
            : static_cast<DbCoord>(request.colId) * placement_.siteWidth;
  const Orient targetOrientation
      = hasOldTarget ? inst->orientation : toPlannerOrient(request.orientation);
