...
bool touchesOrOverlaps(const XInterval& left, const std::vector<XInterval>& rights)
{
...
{
  return std::max<Dbu>((Dbu) 0, std::max(left.xl, right.xl) - std::min(left.xh, right.xh));
}
...
ImplantLayerChecker::ImplantLayerChecker(Grid* grid, Network* network) : DRCChecker(grid), network_(network)
{
  if (grid_ == nullptr) {
    diagnostics_.push_back({"missing_grid", "fatal: ImplantLayerChecker requires an initialized Grid"});
    return;
  }
  if (network_ == nullptr) {
...
void ImplantLayerChecker::buildLayers(PhysDesMgr* desMgr)
{
  ...
  for (const Layer& il : layers_) {
    ...
    // Width
    rules_.emplace_back(Rule(rules_.size(), RuleSource::Width, il.getId(), techLayer.getWidth().getStorage()));
    // Spacing
    rules_.emplace_back(Rule(rules_.size(), RuleSource::Spacing, il.getId(), techLayer.getMinSpacing().getStorage()));
    ...
    for (const eLIB::TechRule& techRule : techLayer.getRuleIter()) {
      if (techRule.getCheck().getType() == eLIB::RuleCheckType::WIDTH_RULE) {
        ...
        //      ; "
        const eLIB::WidthRule* widthRule = dynamic_cast<const eLIB::WidthRule*>(&techRule.getCheck());
        if (widthRule) {
          rules_.push_back(Rule(rules_.size(), RuleSource::Lef58Width, il.getId(), widthRule->getWidth().getStorage()));
          Rule& rule = rules_.back();

          // Set optional clauses
          const std::string& layerName = widthRule->getOtherImplLayerName();
          if (!layerName.empty()) {
          ...
          const std::string& checkGroup = widthRule->getCheckImplantGroup();
          if (!checkGroup.empty()) {
            rule.setCheckGroup(checkGroup);
            uvAssert(layerGroups_.contains(checkGroup));
          }
        }
      } else if (techRule.getCheck().getType() == eLIB::RuleCheckType::SPACING_IMPLANT_RULE) {
        ...
        if (spacingRule) {
          const std::vector<eLIB::SpacingImplantRule::SIItem>& spacingTable = spacingRule->getSpacingImplantTable();
          for (const eLIB::SpacingImplantRule::SIItem& item : spacingTable) {
            rules_.push_back(Rule(rules_.size(), RuleSource::Lef58Spacing, il.getId(), item.minSpacing.getStorage()));
            Rule& rule = rules_.back();
            ...
            if (item.prlOrient == eLIB::RuleCheckOrthoTypeE::HORIZONTAL) {
              rule.setDirection(RuleDirection::Horizontal);
            } else if (item.prlOrient == eLIB::RuleCheckOrthoTypeE::VERTICAL) {
...
bool ImplantLayerChecker::buildRules()
{
  ...
  for (Rule rule : rules_) {
    ...
    if (!rule.getUnsupportedClauses().empty()) {
      diagnostics_.push_back(
          {"skipped_unsupported_rule_clause", makeMessage("unsupported LEF58 clause in rule ", rule.getRuleId())});
    }
    if (rule.getCheckGroup()) {
      if (layerGroups_.find(*rule.getCheckGroup()) == layerGroups_.end()) {
        diagnostics_.push_back({"skipped_missing_rule_parameter", "unknown implant group " + *rule.getCheckGroup()});
      }
    }
  }
  ...
  std::sort(sortedRules_.begin(), sortedRules_.end(), [](const Rule* left, const Rule* right) {
...
bool ImplantLayerChecker::buildMstIntervals()
{
  ...
  // them as context.
  const bool keepInternal = std::any_of(sortedRules_.begin(), sortedRules_.end(), [](const Rule* rule) {
    return rule->getCheckGroup().has_value() || !rule->getIntersectLayers().empty();
  });
  ...
  for (MasterItem& item : masterItems_) {
    ...
    for (size_t i = 0; i < item.shapes.size(); ++i) {
      ...
      if (shapeXl >= shapeXh || shapeYl >= shapeYh) {
        diagnostics_.push_back(
            {"skipped_unsupported_geometry", makeMessage("invalid rectangle shape ", shape.shapeId)});
        ok = false;
        continue;
      }
      if (shapeXl != 0 || shapeXh != width) {
        diagnostics_.push_back(
            {"implant_shape_width_mismatch", makeMessage("implant shape does not span master width ", shape.shapeId)});
        ok = false;
      }
      if (shapeXl < 0 || shapeXh > width || shapeYl < 0 || shapeYh > item.height) {
        diagnostics_.push_back({"shape_outside_macro_boundary", makeMessage("shape outside macro ", shape.shapeId)});
        ok = false;
        continue;
      }
      for (size_t j = i + 1; j < item.shapes.size(); ++j) {
        ...
        const bool xOverlap = std::max(shape.rect._xl, other.rect._xl) < std::min(shape.rect._xh, other.rect._xh);
        const bool yOverlap = std::max(shape.rect._yl, other.rect._yl) < std::min(shape.rect._yh, other.rect._yh);
        if (xOverlap && yOverlap) {
          diagnostics_.push_back({"shape_overlap_in_input", makeMessage("overlap at shape ", shape.shapeId)});
          ok = false;
        }
      }

      const bool macroInternal = shapeXl > 0 && shapeXh < width && shapeYl > 0 && shapeYh < item.height;
      ...
      while (y < shapeYh) {
        ...
        const BandSlot slot = y < bottomEnd ? BandSlot::Bottom : BandSlot::Top;
        const Dbu slotEnd = slot == BandSlot::Bottom ? bottomEnd : rowBase + rowHeight_;
        const Dbu pieceEnd = std::min(shapeYh, slotEnd);
        if (pieceEnd <= y) {
          break;
        }
        const bool fullSlot = y == (slot == BandSlot::Bottom ? rowBase : bottomEnd) && pieceEnd == slotEnd;
        if (!fullSlot) {
          ...
          diagnostics_.push_back({"unsupported_row_band_slot", makeMessage("partial band slot shape ", shape.shapeId)});
        }
        ...
        interval.skipReason = interval.runtimeCheckable ? "" : "macro_internal_after_restructure";
...
void ImplantLayerChecker::buildMasters()
{
  ...
  for (const std::unique_ptr<Master>& masterPtr : network_->getMasters()) {
    ...
    if (!nm) {
      diagnostics_.push_back({"null_network_master", "Network contains a null master slot"});
      continue;
    }
    MasterId mid = nm->getId();
    if (mid < 0 || mid >= masterCount) {
      diagnostics_.push_back({"invalid_network_master_id", makeMessage("master ", mid)});
      continue;
    }
    const eLIB::PhysLibCell* physCell = nm->getPhysLibCell();
    if (physCell == nullptr) {
      diagnostics_.push_back({"missing_physical_master", makeMessage("master ", mid)});
      continue;
    }
    const eLIB::TechSite* techSite = physCell->getTechSite();
    if (techSite == nullptr) {
      diagnostics_.push_back({"missing_master_site", makeMessage("master ", mid)});
      continue;
    }
    ...
    for (const eLIB::PhysLibObs& obs : obsVec) {
      const eLIB::LayerShapeMapT& shapes = obs.getShapes(eUTL::PhysOrientationE::R0);
      ...
      for (const auto& [layerRelId, shapeVec] : shapes) {
        ...
        for (const eLIB::TechShape& techShape : shapeVec) {
          if (techShape.getType() != eLIB::TechShape::RECT) {
            diagnostics_.push_back({"skipped_unsupported_geometry", "master non-RECT shape"});
            continue;
          }
  ...
  for (MasterItem& item : masterItems_) {
    ...
    if (item.siteHeight <= 0) {
      diagnostics_.push_back({"skipped_rebuild_no_site_height",
                              "skipped_rebuild_no_site_height: master " + std::to_string(item.masterId)});
    ...
    for (const MasterShape& rs : item.rawShapes) {
      auto layerIt
          = std::find_if(layers_.begin(), layers_.end(), [&](const Layer& l) { return l.getId() == rs.layer; });
    ...
    if (family == Layer::Vt::Unknown) {
      diagnostics_.push_back({"skipped_rebuild_unknown_family",
                              "skipped_rebuild_unknown_family: master " + std::to_string(item.masterId)});
      item.shapes = item.rawShapes;
      continue;
    }
    ...
    if (familyNLayer < 0 || familyPLayer < 0) {
      diagnostics_.push_back(
          {"skipped_rebuild_missing_layer",
           "skipped_rebuild_missing_layer: master " + std::to_string(item.masterId) + " family missing N or P layer"});
      item.shapes = item.rawShapes;
      continue;
    }
    ...
    for (int row = 0; row < numRows; ++row) {
      ...
      if (row % 2 == 0) {
        ...
        topBandPol = (bottomPolarity == Layer::Polar::N) ? Layer::Polar::P : Layer::Polar::N;
      } else {
        bottomBandPol = (bottomPolarity == Layer::Polar::N) ? Layer::Polar::P : Layer::Polar::N;
      ...
      // Bottom band shape
      {
        MasterShape ms;
        ms.shapeId = shapeId++;
        ms.layer = (bottomBandPol == Layer::Polar::N) ? familyNLayer : familyPLayer;
        Dbu yBase = static_cast<Dbu>(row) * fullRow;
        ms.rect = eUTL::Rect(eUTL::UvDist(static_cast<int64_t>(0)), eUTL::UvDist(yBase), eUTL::UvDist(item.width),
      ...
      // Top band shape
      {
        MasterShape ms;
        ms.shapeId = shapeId++;
        ms.layer = (topBandPol == Layer::Polar::N) ? familyNLayer : familyPLayer;
...
// --------------------------------------------------------------------------------
bool ImplantLayerChecker::check(const Node* node, GridX x, GridY y, const eUTL::PhysOrientation& orient) const
{
  std::vector<CellChangeRecord> fcRecord;
  return check(node, x, y, orient, fcRecord);
}

bool ImplantLayerChecker::check(const Node* node, GridX x, GridY y, const eUTL::PhysOrientation& orient,
...
bool ImplantLayerChecker::repairFillers(const CheckRequest& request, std::vector<CellChangeRecord>& fcRecord) const
{
  ...
  fcRecord.insert(fcRecord.end(), outcome.changes.begin(), outcome.changes.end());
  return true;
}
...
CheckResult ImplantLayerChecker::checkDirect(const CheckRequest& request) const
{
  ...
  if (node == nullptr) {
    result.diagnostics = diagnostics_;
    result.diagnostics.push_back({"unknown_target_instance", makeMessage("instance ", request.instanceId)});
    result.isLegal = false;
    return result;
  }
  if (node->getMaster() == nullptr) {
    result.diagnostics = diagnostics_;
    result.diagnostics.push_back({"target_instance_missing_master", makeMessage("instance ", request.instanceId)});
    result.isLegal = false;
    return result;
  }
  const bool targetHasData = request.masterId >= 0 && request.masterId < static_cast<MasterId>(masterItems_.size())
                             && masterItems_[request.masterId].width > 0;
  if (!targetHasData) {
    result.diagnostics = diagnostics_;
    result.diagnostics.push_back({"unknown_target_master", makeMessage("master ", request.masterId)});
    result.isLegal = false;
    return result;
  }
  ...
  for (const CheckShape& shape : snapshot) {
    if (shape.isCandidate && !slotPolarityOk(shape)) {
      result.diagnostics = diagnostics_;
      result.diagnostics.push_back({"row_slot_polarity_mismatch", makeMessage("instance ", shape.ownerInstanceIds[0])});
      result.isLegal = false;
      return result;
    }
  }
...
std::vector<CheckResult> ImplantLayerChecker::checkAllNodesDirect() const
{
  ...
  auto run_job = [&](size_t i) {
    ...
    if (node == nullptr || node->getMaster() == nullptr) {
      ...
      results[i].diagnostics.push_back({node == nullptr ? "null_network_node" : "target_instance_missing_master",
                                             ...
                                             const std::set<InstanceId>& excludedNodes) const
{
  ...
  if (tgtNode == nullptr || tgtNode->getMaster() == nullptr || siteWidth_ <= 0 || rowHeight_ <= 0) {
    return snapshot;
  }
  ...
  ColId col1 = std::min(request.colId + width + maxRuleValue_ - 1, grid_->getRowSiteCount().v - 1);
    ...
    for (ColId c = col0; c <= col1; c++) {
      ...
      if (p && p->cell) {
        ...
        const CheckShapes& shapes
            = getNodeShape(nodeId, node->getMaster()->getId(), coord.second.v, coord.first.v, node->getOrient(), false);
  ...
  const CheckShapes& shapes
      = getNodeShape(request.instanceId, request.masterId, request.rowId, request.colId, request.orientation, true);
  snapshot.insert(snapshot.end(), shapes.begin(), shapes.end());
  return snapshot;
}

// Collect shape intervals within a guard region including filler overlay changes.
CheckShapes ImplantLayerChecker::getOverlaySnapshot(const CheckRequest& request, const Rect& guardRegion,
                                                    const FillerChanges& fillerChanges, bool useNewFillers,
                                                    const std::set<InstanceId>& excludedNodes) const
{
  CheckShapes snapshot;
  ...
  for (RowId r = row0; r <= row1; r++) {
    for (ColId c = col0; c <= col1; c++) {
      ...
      if (p && p->cell) {
        ...
        const std::pair<GridX, GridY> coord = grid_->gridXY(node);
        const CheckShapes& shapes
            = getNodeShape(nodeId, node->getMaster()->getId(), coord.second.v, coord.first.v, node->getOrient(), true);
        snapshot.insert(snapshot.end(), shapes.begin(), shapes.end());
      }
    }
  }
  ...
  for (const CellChangeRecord& change : fillerChanges) {
    ...
    const PhysOrientation fillerOrientation = useNewFillers ? change.orientation_ : fillerNode->getOrient();
    const CheckShapes& fillerShapes
        = getNodeShape(fillerInstId, fillerMasterId, fillerRowId, fillerColId, fillerOrientation, true);
...
CheckShapes ImplantLayerChecker::mergeShapes(const CheckShapes& rects) const
{
  ...
  std::sort(sorted.begin(), sorted.end(), [](const CheckShape& left, const CheckShape& right) {
  ...
  for (const CheckShape& rect : sorted) {
    if (shapes.empty() || shapes.back().rowId != rect.rowId || shapes.back().bandSlot != rect.bandSlot
        || shapes.back().layer != rect.layer || shapes.back().isCandidate != rect.isCandidate
...
std::vector<CheckOutcome> ImplantLayerChecker::evalRule(const Rule& rule, const CheckShapes& shapes,
  ...
  for (const CheckShape& target : shapes) {
    ...
    for (Relationship rlt : relations_) {
      ...
      if (rule.isWidth() && rlt == Relationship::InterRow) {
        for (const CheckShape& sameRowNeighbor : findNeighbors(target, rule, Relationship::IntraRow, shapes)) {
          if (target.layer == sameRowNeighbor.layer && touchesOrOverlaps(checkTarget.x, sameRowNeighbor.x)) {
      ...
      const std::vector<CheckShape> neighbors = findNeighbors(checkTarget, rule, rlt, shapes);
      if (rule.isWidth() && neighbors.empty() && rlt == Relationship::IntraRow) {
        CheckOutcome outcome;
        outcome.ruleId = rule.getRuleId();
        outcome.ruleSource = rule.getSource();
        ...
        if (touchesOrOverlaps(outcome.xWindow, tgtItvs)) {
          outcome.status
              = outcome.measuredValue < rule.getMinValue() ? OutcomeStatus::Violated : OutcomeStatus::Satisfied;
        } else {
          outcome.status = OutcomeStatus::Skipped;
        }
      ...
      for (const CheckShape& neighbor : neighbors) {
        ...
        if (rule.getExceptCornerTouch() && rlt == Relationship::InterRow && projected == 0) {
          outcome.status = OutcomeStatus::NotApplicable;
          outcomes.push_back(outcome);
          continue;
        }
        ...
        if (!rule.getIntersectLayers().empty() && !isIntersectCoverage(rule, checkTarget, neighbor, shapes)) {
          outcome.status = OutcomeStatus::NotApplicable;
          outcomes.push_back(outcome);
          continue;
        }
        ...
        if (rule.getPrl()) {
          const bool lef58VerticalSpacing
              = rule.getSource() == RuleSource::Lef58Spacing && rlt == Relationship::InterRow;
          const bool lef58HorizontalSpacing
              = rule.getSource() == RuleSource::Lef58Spacing && rlt == Relationship::IntraRow;
          if (!lef58HorizontalSpacing && *rule.getPrl() >= 0 && projected <= *rule.getPrl()) {
            outcome.status = OutcomeStatus::NotApplicable;
            outcomes.push_back(outcome);
            continue;
          }
          if (lef58VerticalSpacing && *rule.getPrl() < 0 && spacing(checkTarget.x, neighbor.x) >= -*rule.getPrl()) {
            outcome.status = OutcomeStatus::NotApplicable;
            outcomes.push_back(outcome);
            continue;
          }
          if (!lef58HorizontalSpacing && !lef58VerticalSpacing && *rule.getPrl() < 0
        ...
        if (rule.isWidth()) {
          ...
          if (rule.getCheckGroup() && !groupFails(rule, checkTarget, neighbor, rlt, effective, shapes)) {
            outcome.status = OutcomeStatus::NotApplicable;
            outcomes.push_back(outcome);
            continue;
          }
          ...
          if (checkTarget.layer == neighbor.layer && touchesOrOverlaps(checkTarget.x, neighbor.x)) {
            outcome.status = OutcomeStatus::NotApplicable;
            outcomes.push_back(outcome);
            continue;
          }
          outcome.xWindow = gap(checkTarget.x, neighbor.x);
          outcome.measuredValue = rule.getSource() == RuleSource::Lef58Spacing && rlt == Relationship::InterRow
...
// and relationship.
CheckShapes ImplantLayerChecker::findNeighbors(const CheckShape& target, const Rule& rule, Relationship rlt,
                                               const CheckShapes& shapes) const
{
  ...
  const bool returnSameRowRuns = rule.isWidth() && rlt == Relationship::InterRow;
  ...
  for (const CheckShape& shape : shapes) {
    ...
    if (rlt == Relationship::IntraRow && rule.isWidth() && !touchesOrOverlaps(target.x, shape.x)) {
      continue;
    }
  ...
  std::sort(neighbors.begin(), neighbors.end(), [](const CheckShape& left, const CheckShape& right) {
    if (left.rowId != right.rowId) {
      return left.rowId < right.rowId;
    }
    if (left.bandSlot != right.bandSlot) {
  ...
  for (const CheckShape& shape : neighbors) {
    if (runs.empty() || runs.back().rowId != shape.rowId || runs.back().bandSlot != shape.bandSlot
        || runs.back().layer != shape.layer || !touchesOrOverlaps(runs.back().x, shape.x)) {
  ...
  runs.erase(std::remove_if(runs.begin(), runs.end(),
                            [&target](const CheckShape& shape) { return !overlaps(target.x, shape.x); }),
             runs.end());
  return runs;
}

// Check if one outcome context is contained within another (for suppression).
bool ImplantLayerChecker::contained(const CheckOutcome& specific, const CheckOutcome& broad) const
{
  if (specific.ruleSource != broad.ruleSource || specific.primaryLayer != broad.primaryLayer
      || specific.secondaryLayer != broad.secondaryLayer || specific.relationship != broad.relationship
  ...
  return specific.xWindow.xl >= broad.xWindow.xl && specific.xWindow.xh <= broad.xWindow.xh;
}

// Convert violated outcomes into Violation objects, suppressing contained ones.
std::vector<Violation> ImplantLayerChecker::makeViolations(const std::vector<CheckOutcome>& outcomes,
                                                           const CheckShapes& shapes) const
{
  ...
  for (size_t broadIndex = 0; broadIndex < outcomes.size(); ++broadIndex) {
    ...
    for (const CheckOutcome& specific : outcomes) {
      ...
      if (specificRule.getContainmentGroup() != broadRule.getContainmentGroup()) {
        continue;
      }
      if (std::find(specificRule.getContainedByRuleIds().begin(), specificRule.getContainedByRuleIds().end(),
                    broad.ruleId)
  ...
  for (size_t i = 0; i < outcomes.size(); ++i) {
    ...
    if (outcome.status != OutcomeStatus::Violated || suppressed.find(i) != suppressed.end()) {
      continue;
    }
    Violation violation;
    ...
    auto targetIt
        = std::find_if(shapes.begin(), shapes.end(), [&](const CheckShape& s) { return s.id == outcome.targetId; });
    if (targetIt != shapes.end()) {
      XInterval mergedX = targetIt->x;
      const bool isWidthInterRow
          = outcome.ruleSource == RuleSource::Width && outcome.relationship == Relationship::InterRow;
      if (isWidthInterRow) {
        for (const CheckShape& sameRow : shapes) {
          ...
          if (sameRow.rowId != targetIt->rowId || sameRow.bandSlot != targetIt->bandSlot) {
            continue;
          }
    ...
    auto shapeIt = std::find_if(shapes.begin(), shapes.end(), [&](const CheckShape& s) {
      return outcome.neighborId.has_value() && s.id == *outcome.neighborId;
    });
    violation.neighborInterval = shapeIt != shapes.end() ? shapeIt->x : violation.targetInterval;
  ...
  std::sort(violations.begin(), violations.end(), [](const Violation& left, const Violation& right) {
                                                    ...
                                                    const FillerChanges& fillerChanges) const
{
  ...
  if (targetNode == nullptr) {
    ...
    diagnostics.push_back({"target_instance_missing_master", makeMessage("instance ", request.instanceId)});
  }
  const bool targetHasData = request.masterId >= 0 && request.masterId < static_cast<MasterId>(masterItems_.size())
                             && masterItems_[request.masterId].width > 0;
  if (!targetHasData) {
  ...
  for (const CellChangeRecord& change : fillerChanges) {
    if (change.op_ != OpType::Replace) {
      diagnostics.push_back({"unsupported_cell_change_operation", "filler overlay supports Replace records only"});
      continue;
    }
    const LeafCellID* cellId = cellChangeLeafCellId(change);
    if (cellId == nullptr) {
      diagnostics.push_back({"changed_cell_data_not_leaf_id", "Replace record must identify an existing LeafCellID"});
      continue;
    }
    const InstanceId fillerInstId = network_->getNodeId(*cellId);

    if (!seen.insert(fillerInstId).second) {
      diagnostics.push_back({"duplicate_filler_change", makeMessage("duplicate filler change ", fillerInstId)});
      continue;
    }
    if (fillerInstId == request.instanceId) {
      diagnostics.push_back(
          {"target_cannot_be_changed_filler", makeMessage("target cannot be changed filler ", fillerInstId)});
    }

    const Node* fillerNode = network_->getNode(fillerInstId);
    if (!fillerNode) {
      diagnostics.push_back({"unknown_filler_instance", makeMessage("unknown filler instance ", fillerInstId)});
      continue;
    }
    if (fillerNode->getMaster() == nullptr) {
      diagnostics.push_back({"filler_instance_missing_master", makeMessage("filler instance ", fillerInstId)});
      continue;
    }
    if (!fillerNode->isFiller()) {
      diagnostics.push_back(
          {"changed_instance_not_filler", makeMessage("changed instance is not filler ", fillerInstId)});
    }

    const MasterId newMasterId = network_->getMasterId(change.new_lib_cell_);
    const bool newMasterHasData = newMasterId >= 0 && newMasterId < static_cast<MasterId>(masterItems_.size())
                                  && masterItems_[newMasterId].width > 0;
    if (!newMasterHasData) {
      diagnostics.push_back({"unknown_filler_master", makeMessage("unknown filler master ", newMasterId)});
      continue;
    }
    const Master* newMaster = network_->getMaster(newMasterId);
    if (newMaster == nullptr || !newMaster->isFiller()) {
      diagnostics.push_back(
          {"replacement_master_not_filler", makeMessage("replacement master is not filler ", newMasterId)});
    }

    const MasterId oldMasterId = fillerNode->getMaster()->getId();
    const bool oldMasterHasData = oldMasterId >= 0 && oldMasterId < static_cast<MasterId>(masterItems_.size())
    ...
    if (oldMasterHasData && newMasterHasData
      ...
      diagnostics.push_back(
          {"replacement_footprint_mismatch", makeMessage("replacement footprint mismatch ", fillerInstId)});
    }
  }
  return diagnostics;
}

bool ImplantLayerChecker::touchesInstance(const Violation& violation, InstanceId instanceId) const
{
  return std::find(violation.instances.begin(), violation.instances.end(), instanceId) != violation.instances.end();
}

bool ImplantLayerChecker::containsViolation(const Violation& oldViolation, const Violation& newViolation) const
{
  if (oldViolation.hash != newViolation.hash || !contains(oldViolation.xWindow, newViolation.xWindow)) {
    return false;
  }
  for (InstanceId instanceId : newViolation.instances) {
    if (std::find(oldViolation.instances.begin(), oldViolation.instances.end(), instanceId)
...
bool ImplantLayerChecker::isInGuard(const XInterval& xWindow, const RowIdVec& rowIds, const Rect& guard) const
{
  ...
  for (RowId rowId : rowIds) {
    ...
    if (overlaps(XInterval{rowYl, rowYh}, XInterval{guard.getYL().getStorage(), guard.getYH().getStorage()})
...
// target and neighbor.
bool ImplantLayerChecker::isIntersectCoverage(const Rule& rule, const CheckShape& target, const CheckShape& neighbor,
                                              const CheckShapes& shapes) const
{
  const XInterval gap{std::min(target.x.xh, neighbor.x.xh), std::max(target.x.xl, neighbor.x.xl)};
  for (LayerId layer : rule.getIntersectLayers()) {
    bool covered = false;
    for (const CheckShape& shape : shapes) {
      if (shape.rowId == target.rowId && shape.bandSlot == target.bandSlot && shape.layer == layer
          && shape.x.xl <= gap.xl && shape.x.xh >= gap.xh) {
        covered = true;
        break;
      }
    }
...
bool ImplantLayerChecker::groupFails(const Rule& rule, const CheckShape& target, const CheckShape& neighbor,
                                     Relationship rlt, const XInterval& xWindow, const CheckShapes& shapes) const
{
  ...
  auto inGroup = [&](LayerId layer) {
    return std::find(layersIt->second.begin(), layersIt->second.end(), layer) != layersIt->second.end();
  };
  auto groupShapesFor = [&](RowId rowId, BandSlot bandSlot) {
    CheckShapes groupShapes;
    int nextId = 1;
    for (const CheckShape& shape : shapes) {
      if (shape.rowId != rowId || shape.bandSlot != bandSlot || !inGroup(shape.layer)) {
        continue;
      }
  ...
  Dbu maxWidth = 0;
  const std::vector<CheckShape> targetShapes = groupShapesFor(target.rowId, target.bandSlot);
  if (rlt == Relationship::InterRow) {
    const std::vector<CheckShape> neighborShapes = groupShapesFor(neighbor.rowId, neighbor.bandSlot);
...
CheckShapes ImplantLayerChecker::mergeGroupShapes(const CheckShapes& rawShapes, bool isCandidate) const
{
  ...
  for (const CheckShape& raw : sorted) {
    if (merged.empty() || merged.back().rowId != raw.rowId || merged.back().bandSlot != raw.bandSlot
...
// --------------------------------------------------------------------------------
void ImplantLayerChecker::parseLayerName(const std::string& name, Layer::Vt& family, Layer::Polar& polarity)
{
  ...
  polarity = ((polStr == "P" || polStr == "p") ? Layer::Polar::P : Layer::Polar::N);
...
std::vector<CheckResult> ImplantLayerChecker::checkPlaceWithOverlays(
    const CheckRequest& request, const Rect& guardRegion, const std::vector<FillerChanges>& fillerChanges) const
{
  ...
  if (!targetDiagnostics.empty()) {
    for (CheckResult& result : results) {
      result.isLegal = false;
      result.diagnostics = diagnostics_;
      result.diagnostics.insert(result.diagnostics.end(), targetDiagnostics.begin(), targetDiagnostics.end());
    }
    return results;
  }
  const std::vector<Violation> oldViolations = checkOverlayRegion(request, guardRegion, {}, false).violations;
  auto run_job
      = [&](int i) { results[i] = checkPlaceWithOverlay(request, guardRegion, fillerChanges[i], oldViolations); };
  if (DrcUtil::getArena()) {
...
// the target.
CheckResult ImplantLayerChecker::checkPlaceWithOverlay(const CheckRequest& request, const Rect& guardRegion,
                                                       const FillerChanges& fillerChanges,
                                                       const std::vector<Violation>& oldViolations) const
{
  CheckResult result;
  result.diagnostics = diagnostics_;
  const std::vector<Diagnostic> requestDiagnostics = validateOverlayRequest(request, fillerChanges);
  result.diagnostics.insert(result.diagnostics.end(), requestDiagnostics.begin(), requestDiagnostics.end());
  if (!requestDiagnostics.empty()) {
    result.isLegal = false;
    return result;
  }

  const CheckResult overlay = checkOverlayRegion(request, guardRegion, fillerChanges, true);
  result.diagnostics.insert(result.diagnostics.end(), overlay.diagnostics.begin(), overlay.diagnostics.end());
  ...
  for (const Violation& violation : overlay.violations) {
    ...
    const bool isOld = std::any_of(
        oldViolations.begin(), oldViolations.end(),
        [&violation, this](const Violation& oldViolation) { return containsViolation(oldViolation, violation); });
    if (!isOld) {
      blocking.push_back(violation);
    }
  }
  ...
  result.isLegal = result.violations.empty() && requestDiagnostics.empty() && overlay.diagnostics.empty();
  return result;
}

// Run all rules within a guard region, accounting for old vs new filler masters.
CheckResult ImplantLayerChecker::checkOverlayRegion(const CheckRequest& request, const Rect& guardRegion,
                                                    const FillerChanges& fillerChanges, bool useNewFillers) const
{
  ...
  Node* node = network_->getNode(request.instanceId);
  if (node == nullptr || node->getMaster() == nullptr) {
    result.diagnostics.push_back({node == nullptr ? "unknown_target_instance" : "target_instance_missing_master",
  ...
  const CheckShapes& snapshot = getOverlaySnapshot(request, guardRegion, fillerChanges, useNewFillers, excludedNodes);
  for (const CheckShape& shape : snapshot) {
    InstanceId id = shape.ownerInstanceIds[0];
    if (shape.isCandidate && id == request.instanceId && !slotPolarityOk(shape)) {
      result.diagnostics.push_back({"row_slot_polarity_mismatch", makeMessage("instance ", id)});
      result.isLegal = false;
      return result;
    }
  }
  ...
  result.violations = makeViolations(outcomes, shapes);
  result.violations.erase(std::remove_if(result.violations.begin(), result.violations.end(),
                                         [&guardRegion, this](const Violation& violation) {
                                           return !isInGuard(violation.xWindow, violation.rowIds, guardRegion);
                                         }),
...
std::string Violation::toString(Dbu siteWidth) const
{
  ...
  std::stringstream ss;
  ss << "  " << layerName << " " << dpl2::ipl::toString(ruleSource) << " " << dpl2::ipl::toString(relationship);

  if (!instances.empty()) {
    ss << "  merged=[" << toSites(targetInterval.xl) << ',' << toSites(targetInterval.xh) << ']';
    if (neighborInterval.xl != targetInterval.xl || neighborInterval.xh != targetInterval.xh) {
      ss << "  neighbor=[" << toSites(neighborInterval.xl) << ',' << toSites(neighborInterval.xh) << ']';
    }
  }

  ss << "  measured=" << toSites(measuredValue) << 's' << "  required=" << toSites(requiredValue) << 's' << "  x=["
...
void ImplantLayerChecker::printStats(std::ostream& os, bool isShort) const
{
  ...
  if (!isShort) {
    for (const Layer& il : layers_) {
      os << "  LayerId=" << il.getId() << " name=\"" << il.getName() << "\"" << " family=";
      switch (il.getVt()) {
        case Layer::Vt::S:
          os << "VTS";
          break;
  ...
  if (!isShort) {
    for (const Rule& rule : rules_) {
      ...
      os << " primaryLayer=" << rule.getPrimaryLayer() << " minValue=" << rule.getMinValue();
  ...
  if (!isShort) {
    for (const MasterItem& m : masterItems_) {
      if (m.width == 0)
        continue;
      os << "  MasterId=" << m.masterId << " width=" << m.width << " height=" << m.height
         << " siteHeight=" << m.siteHeight << " rawShapes=" << m.rawShapes.size()
         << " rebuiltShapes=" << m.shapes.size() << " : <";
      for (const MasterShape& shape : m.shapes) {
        os << "(" << shape.shapeId << ",L" << shape.layer << ")" << shape.rect.toString() << " ";
      }
      os << ">\n";
  ...
  if (!isShort) {
    for (const std::unique_ptr<Node>& nodePtr : network_->getNodes()) {
      ...
      os << "  InstanceId=" << node->getId() << " masterId=" << node->getMaster()->getId() << " coord=<" << rowId
         << ", " << colId << ">" << " orient=";
