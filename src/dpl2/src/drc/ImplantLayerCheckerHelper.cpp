...
void ImplantLayerCheckerHelper::initialize(const ImplantInput& input)
{
    ...
    for (size_t i = 0; i < input.masters.size(); ++i) {
        ...
        master->setFiller(configuredFiller || placedFiller
                          || input.masters[i].isFiller);
...
void ImplantLayerCheckerHelper::initChecker(ImplantLayerChecker& checker)
{
    ...
    checker.diagnostics_.erase(
        ...
        checker.diagnostics_.end());
    // Preserve the stable (and potentially sparse) Network master ids.
    checker.masterItems_.clear();
    for (const MasterItem& inputMaster : inputMasters_) {
        MasterItem item = inputMaster;
        ...
        checker.masterItems_[item.masterId] = std::move(item);
    }
                                     ...
                                     const ImplantLayerChecker& checker) const
{
    ...
    // For the real-design case, checker runs against a design accessed via
    // its Network. The PlacedInst fields (rowId, colId) are
    ...
    if (checker.network_) {
        for (const auto& [nodeId, nodePtr] : checker.network_->getNodes()) {
            ...
            pi.masterId = node->getMaster() ? node->getMaster()->getId()
                                            : MasterId(-1);
    ...
    const int written = gzwrite(
        gz, data.data(), static_cast<unsigned int>(data.size()));
    const int closeResult = gzclose(gz);
    return written == static_cast<int>(data.size()) && closeResult == Z_OK;
}
...
ImplantInput ImplantLayerCheckerHelper::load(const std::string& filePath)
{
    ...
    std::string data;
    while (true) {
        const int got = gzread(
            gz, buf.data(), static_cast<unsigned int>(buf.size()));
        if (got <= 0) {
            break;
        }
    ...
    if (version >= 5) {
        int basePolar = 0;
        if (!(in >> section >> basePolar) || section != "base_polar") {
            return ImplantInput();
        }
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        int orientation = 0;
        bool isFiller = false;
        if (!(in >> inst.instanceId >> inst.masterId >> inst.rowId
              >> inst.colId >> orientation >> isFiller)) {
            return ImplantInput();
        }
