...
void ImplantLayerCheckerHelper::initialize(const ImplantInput& input)
{
  ...
  grid_->core_ = eUTL::Rect(eUTL::UvDist(0), eUTL::UvDist(0), eUTL::UvDist(siteWidth.v * siteCount.v),
  ...
  for (size_t i = 0; i < input.masters.size(); ++i) {
    ...
    const bool isFiller = std::any_of(
        ...
        [i](const PlacedInst& placed) { return placed.masterId == static_cast<MasterId>(i) && placed.isFiller; });
  ...
  for (const PlacedInst& pi : input.placedInsts) {
    ...
    if (pi.masterId >= 0 && pi.masterId < static_cast<MasterId>(network_->getMasters().size())) {
...
void ImplantLayerCheckerHelper::initChecker(ImplantLayerChecker& checker)
{
  ...
  checker.diagnostics_.erase(
      std::remove_if(checker.diagnostics_.begin(), checker.diagnostics_.end(),
                     [](const Diagnostic& diagnostic) { return diagnostic.status == "missing_grid_phys_des_mgr"; }),
...
bool ImplantLayerCheckerHelper::dump(const std::string& filePath, const ImplantLayerChecker& checker) const
{
  ...
  for (const Layer& layer : checker.layers_) {
    out << layer.getId() << ' ' << std::quoted(layer.getName()) << ' ' << enumInt(layer.getVt()) << ' '
        << enumInt(layer.getPolar()) << "\n";
  }
  ...
  for (const Rule& rule : checker.rules_) {
    out << rule.getRuleId() << ' ' << enumInt(rule.getSource()) << ' ' << rule.getPrimaryLayer() << ' ';
    dumpOptional(out, rule.getSecondaryLayer());
    out << ' ' << rule.getMinValue() << ' ' << enumInt(rule.getDirection()) << ' ';
    dumpOptional(out, rule.getPrl());
    out << ' ' << rule.getZeroPrl() << ' ' << rule.getExceptAbutted() << ' ' << rule.getExceptCornerTouch() << ' ';
  ...
  for (const MasterItem& master : checker.masterItems_) {
    out << master.masterId << ' ' << master.width << ' ' << master.height << ' ' << master.isFiller << ' '
        << master.siteHeight << ' ' << master.shapes.size() << "\n";
    for (const MasterShape& shape : master.shapes) {
      out << shape.masterId << ' ' << shape.shapeId << ' ' << shape.layer << ' ' << shape.rect.getXL().getStorage()
          << ' ' << shape.rect.getYL().getStorage() << ' ' << shape.rect.getXH().getStorage() << ' '
  ...
  if (checker.network_) {
    for (const std::unique_ptr<Node>& nodePtr : checker.network_->getNodes()) {
      ...
      pi.masterId = node->getMaster() ? node->getMaster()->getId() : MasterId(-1);
  ...
  for (const PlacedInst& inst : placedInsts) {
    out << inst.instanceId << ' ' << inst.masterId << ' ' << inst.rowId << ' ' << inst.colId << ' '
...
ImplantInput ImplantLayerCheckerHelper::load(const std::string& filePath)
{
  ...
  std::string tag;
  int version = 0;
  if (!(in >> tag >> version) || tag != "ImplantLayerCheckerDump" || (version != 1 && version != 2 && version != 3)) {
    return ImplantInput();
  }
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    if (!(in >> ruleId >> source >> primaryLayer) || !loadOptional(in, secondaryLayer) || !(in >> minValue >> direction)
        || !loadOptional(in, prl) || !(in >> zeroPrl >> exceptAbutted >> exceptCornerTouch) || !loadOptional(in, length)
        || !loadOptional(in, checkGroup) || !loadVector(in, intersectLayers)) {
      return ImplantInput();
    }
    ...
    if (!loadStringVector(in, unsupportedClauses) || !loadOptional(in, containmentGroup)
        || !loadVector(in, containedByRuleIds) || !(in >> specificityRank)) {
      return ImplantInput();
    }
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    bool isFiller = false;
    Dbu siteHeight = 0;
    if (!(in >> master.masterId >> master.width >> master.height >> isFiller >> siteHeight >> shapeCount)) {
      return ImplantInput();
    }
    ...
    for (size_t s = 0; s < shapeCount; ++s) {
      ...
      if (!(in >> shape.masterId >> shape.shapeId >> shape.layer >> xl >> yl >> xh >> yh)) {
        return ImplantInput();
      }
      shape.rect = ::Rect(eUTL::UvDist(xl), eUTL::UvDist(yl), eUTL::UvDist(xh), eUTL::UvDist(yh));
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    int orientation = 0;
    bool isFiller = false;
    if (!(in >> inst.instanceId >> inst.masterId >> inst.rowId >> inst.colId >> orientation >> isFiller)) {
      return ImplantInput();
    }
