...
constexpr std::array<const char*, (unsigned) RuleSource::Count> kRuleSourceNames{"WIDTH", "SPACING", "LEF58_WIDTH",
                                                                                 "LEF58_SPACING"};
...
class Layer
{
 public:
  ...
  Layer() = default;
  Layer(LayerId id, const std::string& name, Vt vt, Polar polar) : id_(id), name_(name), vt_(vt), polar_(polar) {}
...
class Rule
{
 public:
  Rule() = default;
  Rule(int id, RuleSource rs, LayerId l1, Dbu v) : ruleId_(id), source_(rs), primaryLayer_(l1), minValue_(v) {}
  ...
  ADD_SETTER_GETTER_PP(std::vector<std::string>, UnsupportedClauses, unsupportedClauses_);
...
class ImplantLayerChecker final : public DRCChecker
{
 public:
  ...
  bool check(const Node* cell, GridX x, GridY y, const eUTL::PhysOrientation& orient) const override;
 ...
 private:
  ...
  bool isIntersectCoverage(const Rule& rule, const CheckShape& target, const CheckShape& neighbor,
                           const CheckShapes& shapes) const;

  CheckShapes getSnapshot(const CheckRequest& request, const std::set<InstanceId>& excludedNodes) const;
  CheckShapes getOverlaySnapshot(const CheckRequest& request, const Rect& guardRegion,
                                 const FillerChanges& fillerChanges, bool useNewFillers,
                                 const std::set<InstanceId>& excludedNodes) const;
  CheckShapes getNodeShape(InstanceId instanceId, MasterId masterId, RowId rowId, ColId colId,
                           PhysOrientation orientation, bool isCandidate) const;
  CheckShapes mergeShapes(const CheckShapes& rects) const;
  CheckShapes findNeighbors(const CheckShape& target, const Rule& rule, Relationship relationship,
                            const CheckShapes& shapes) const;
  std::vector<CheckOutcome> evalRule(const Rule& rule, const CheckShapes& shapes,
                                     const std::vector<XInterval>& tgtItvs) const;
  std::vector<Violation> makeViolations(const std::vector<CheckOutcome>& outcomes, const CheckShapes& shapes) const;
  CheckResult checkPlaceWithOverlay(const CheckRequest& request, const Rect& guardRegion,
                                    const FillerChanges& fillerChanges,
                                    const std::vector<Violation>& oldViolations) const;
  CheckResult checkOverlayRegion(const CheckRequest& request, const Rect& guardRegion,
  ...
  bool groupFails(const Rule& rule, const CheckShape& target, const CheckShape& neighbor, Relationship relationship,
  ...
  const std::array<Relationship, 2> relations_ = {Relationship::IntraRow, Relationship::InterRow};
};
