...
{
    return std::array<LayerId, 3>{F1_LAYER, F2_LAYER, F3_LAYER}
        [layerIndex];
}
...
ImplantInput input()
{
    ...
    input.masters = {
        master(C1_MASTER, 1, F1_LAYER, false),
...
{
    return std::any_of(result.violations.begin(),
                       result.violations.end(),
                       [ruleId, relationship, instanceIds](
...
TEST(ImplantCheckerOverlayTest, DenseCaseIntraRowWidth)
{
    const CheckRequest target =
        request(INTRA_WIDTH_ROW, INTRA_WIDTH_COL);
    const std::vector<CheckResult> results =
        check(target,
              {{FillerCellRecord{OpType::Replace, leafCellId(INTRA_WIDTH_ROW, 11),
                                UvDist(0), UvDist(0), LibCellID(),
               ...
               {FillerCellRecord{OpType::Replace, leafCellId(INTRA_WIDTH_ROW, 11),
                                UvDist(0), UvDist(0), LibCellID(),
                                libCellId(F1_FILL_MASTER)},
                FillerCellRecord{OpType::Replace, leafCellId(NEW_INTRA_WIDTH_ROW,
                                101), UvDist(0), UvDist(0), LibCellID(),
                                libCellId(F2_FILL_MASTER)}}});
...
TEST(ImplantCheckerOverlayTest, DenseCaseInterRowWidth)
{
    ...
    const std::vector<CheckResult> results =
        check(target,
              ...
              leafCellId(INTER_WIDTH_NEIGHBOR_ROW, 31),
                                UvDist(0), UvDist(0),
                                LibCellID(), libCellId(F1_FILL_MASTER)}},
              {FillerCellRecord{OpType::Replace,
              leafCellId(INTER_WIDTH_NEIGHBOR_ROW, 31),
                                UvDist(0), UvDist(0),
                                LibCellID(), libCellId(F2_FILL_MASTER)}},
              {FillerCellRecord{OpType::Replace,
              leafCellId(INTER_WIDTH_NEIGHBOR_ROW, 31),
                                UvDist(0), UvDist(0),
                                LibCellID(), libCellId(F1_FILL_MASTER)},
              FillerCellRecord{OpType::Replace,
              leafCellId(NEW_INTER_WIDTH_BOTTOM_ROW,
                                NEW_INTER_WIDTH_COL),
                                UvDist(0), UvDist(0),
                                LibCellID(), libCellId(F1_FILL_MASTER)}}});
    ...
    EXPECT_TRUE(hasViolation(results[2],
                             F1_WIDTH_RULE,
                             Relationship::InterRow,
                             {instId(NEW_INTER_WIDTH_TOP_ROW,
                                     NEW_INTER_WIDTH_COL),
                              instId(NEW_INTER_WIDTH_BOTTOM_ROW,
                                     NEW_INTER_WIDTH_COL)}));
    expectOldUnrelatedFiltered(results[0]);
}
...
{
    const CheckRequest target =
        request(INTRA_SPACING_ROW, INTRA_SPACING_COL);
    const InstanceId neighbor =
    ...
    const std::vector<CheckResult> results =
        check(target,
              ...
              leafCellId(INTRA_SPACING_ROW, 52),
                                UvDist(0), UvDist(0),
                                LibCellID(), libCellId(F1_FILL_MASTER)}},
               {FillerCellRecord{OpType::Replace, leafCellId(INTRA_SPACING_ROW, 52),
                                UvDist(0), UvDist(0),
                                LibCellID(), libCellId(F2_FILL_MASTER)}},
               {FillerCellRecord{OpType::Replace,
               leafCellId(INTRA_SPACING_ROW, 52),
    ...
    EXPECT_TRUE(hasViolation(results[2],
                             F1_SPACING_RULE,
                             Relationship::IntraRow,
                             {instId(NEW_INTRA_SPACING_ROW,
                                     NEW_INTRA_SPACING_COL),
                             instId(NEW_INTRA_SPACING_ROW,
                                     NEW_INTRA_SPACING_COL + 4)}));
    expectOldUnrelatedFiltered(results[0]);
}
...
TEST(ImplantCheckerOverlayTest, DenseCaseInterRowSpacing)
{
    ...
    EXPECT_TRUE(hasViolation(results[2],
                             F1_SPACING_RULE,
                             Relationship::InterRow,
                             {instId(NEW_INTER_SPACING_TOP_ROW,
                                     NEW_INTER_SPACING_COL),
                              instId(NEW_INTER_SPACING_BOTTOM_ROW,
                                     NEW_INTER_SPACING_COL + 4)}));
    expectOldUnrelatedFiltered(results[0]);
}
