...
struct Rect
{
  ...
  int xMin() const { return xlo; }
  int yMin() const { return ylo; }
  int xMax() const { return xhi; }
  int yMax() const { return yhi; }
};
...
enum class dbMasterType
{
  CORE,
  CORE_SPACER,
  BLOCK
};
enum class dbTechLayerType
{
  ROUTING,
  IMPLANT,
  CUT
};
enum class dbOrientType
{
  R0,
  MX,
  MY,
...
class dbTechLayer
{
 public:
  std::string name_;
  dbTechLayerType type_ = dbTechLayerType::ROUTING;
...
class dbMaster
{
 public:
  std::string name_;
  dbMasterType type_ = dbMasterType::CORE;
  bool block_ = false;
  int w_ = 0, h_ = 0;
  std::vector<dbBox*> obs_;
  std::string getName() const { return name_; }
...
class dbInst
{
 public:
  ...
  dbMaster* master_ = nullptr;
  std::string name_;
  int x_ = 0, y_ = 0;
  bool placed_ = true, fixed_ = false, destroyed_ = false;
  ...
  static dbInst* create(dbBlock* b,
                        dbMaster* m,
                        const char* nm,
                        bool physical_only);
